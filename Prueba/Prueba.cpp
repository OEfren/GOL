...
int** evaluarMatriz(int** matriz, int numRows, int numCols)
{
	...
	// Primero simplemene dejamos vivos a la siguiente geenración
	for (int row = 0; row < numRows; row++)
	{
		for (int col = 0; col < numCols; col++)
		{
			...
			if ((vecinos == 2 || vecinos == 3) && matriz[row][col] == 1)
			{
				newMatriz[row][col] = 1;
			}
			else if (newMatriz[row][col] == 0 && vecinos == 3) {
				newMatriz[row][col] = 1;
			}
			else if (newMatriz[row][col] == 1 && vecinos > 3) {
				newMatriz[row][col] = 0;
			}
		}
	}
